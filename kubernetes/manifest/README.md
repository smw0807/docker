...
파드 내에 초기화만 담당하는 컨테이너를 설정할 수도 있다. 그러면 초기화만 수행하는 컨테이너와 요청을 처리하는 컨테이너를 별도로 개발하여 각각 재사용할 수 있다. 예를 들어, 스토리지를 마운트할 때 ‘스토리지 안에 새로운 디렉터리를 만들고, 소유자를 변경한 후 데이터를 저장’하는 것과 같은 초기화 처리를 전담할 수 있다.
...
initContainer는 초기화 전용 컨테이너이다.
volumeMounts를 보면 초기화 전용 컨테이너가 공유 볼륨 /mnt를 마운트하고 args의 명령어로 디렉터리 /mnt/html을 만들고 소유자를 변경한다.
이 컨테이너는 명령어를 실행하고 바로 종료한다.
이후 메인 컨테이너가 기동하여 공유 볼륨을 마운트 한다.
